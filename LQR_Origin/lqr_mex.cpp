...
// 将 MATLAB 的 mxArray 转换为内部 Matrix 表示。
static Matrix mxArrayToMatrix(const mxArray* arr) {
    if (!mxIsDouble(arr) || mxIsComplex(arr)) {
        mexErrMsgIdAndTxt("lqr_mex:invalidInput", "A, B, Q and R must be real double arrays.");
    }

    const mwSize rows = mxGetM(arr);
    const mwSize cols = mxGetN(arr);
    double* data = static_cast<double*>(mxGetPr(arr));

    Matrix m(static_cast<int>(rows), static_cast<int>(cols), 0.0);
...
// 将内部 Matrix 转换回 MATLAB 的 mxArray。
static mxArray* matrixToMxArray(const Matrix& m) {
...
// MEX 入口：校验输入、求解 LQR，并返回 K 和可选的 P。
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    if (nrhs != 4) {
        mexErrMsgIdAndTxt("lqr_mex:nargin", "Usage: [K, P] = lqr_mex(A, B, Q, R)");
    }
    if (nlhs > 2) {
        mexErrMsgIdAndTxt("lqr_mex:nargout", "At most two outputs are supported.");
    }

    try {
        const Matrix A = mxArrayToMatrix(prhs[0]);
        const Matrix B = mxArrayToMatrix(prhs[1]);
        const Matrix Q = mxArrayToMatrix(prhs[2]);
        const Matrix R = mxArrayToMatrix(prhs[3]);

        Matrix K;
        Matrix P;
        if (!solveDiscreteLQR(A, B, Q, R, K, P)) {
            mexErrMsgTxt("LQR solve failed.");
        }

        plhs[0] = matrixToMxArray(K);
        if (nlhs >= 2) {
            plhs[1] = matrixToMxArray(P);
        }
